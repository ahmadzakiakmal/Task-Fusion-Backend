...
INSERT INTO task (project_id, title, description, milestone, deadline, created) 
VALUES ($project_id,  '${task.title}', '${task.description}', '${task.milestone}', '$deadline', '$created');
...
''');
 ...
 final result =  await dbConnection.db.execute('''
      DELETE FROM task WHERE id = $taskId AND project_id = $project_id
...
''');
 ...
 final result =  await dbConnection.db.execute('''
      SELECT * FROM task WHERE id = $taskId AND project_id = $project_id
